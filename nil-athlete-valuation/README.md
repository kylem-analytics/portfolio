...
4. **Live auction** — bid against every other school in the class in real time, in a single class-wide auction for the full 25-athlete pool, tracking wins, losses, and unexpected events ("curveballs") as they happened
5. **Post-auction audit** — evaluate whether the model actually led to better decisions than gut instinct would have, scored against a championship simulation that converted every school's final roster into a performance ranking

The live auction was the actual test of the model: every school in the class bid against each other for the same pool of athletes, so the valuation framework had to hold up under real competition, not just on paper. USF's roster came out **#1 in the class's championship simulation** — the model identified the right targets (most notably quarterback Marcus DeLeon, won for $500,000 against a model valuation near $1M), and disciplined bidding on the rest of the board meant USF got the most on-field value per dollar spent of any school in the auction.
...
- **The model's top targets won the auction, and the results held up against the rest of the class.** Marcus DeLeon (QB) was the model's highest-valued athlete and was acquired for $500,000 against a model valuation near $1M — the single biggest value find of the auction. Shooting guard Andre Simms was paid above model value ($3M against a much lower model estimate) as a deliberate in-the-moment decision to fill a critical gap with budget still available. Across the full class-wide auction, that combination of disciplined value-finding and targeted overpaying on real needs produced the #1-ranked roster in the championship simulation.
